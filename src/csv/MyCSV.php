...
class MyCSV
{
    ...
    /**
     * The field delimiter for separating values in the CSV file.
     * Default is ","
    ...
    function read($tablename, $length = 10000)
    {
        ...
        while (count($this->fields) < 2) {
            ...
            if (! $delimiters = substr($delimiters, 1))
                break;
            rewind($this->_fp);
            $this->fields = fgetcsv($this->_fp, $length, $this->delimiter);
        }
        ...
        for ($i = count($this->fields) - 1; $i > - 1; $i --) {
            if (strcasecmp($this->fields[$i], "id") == 0)
                break;
        }
        $lastId = 0;
    ...
    /**
     * Deletes a table row specified by the <i>id</i>.
     * Deletes all rows if no
     * <i>id</i> is given.
     *
     * @param
     *            id mixed
    ...
    {
        if (is_array($field) || strcasecmp($field, "id") == 0)
            return false;
    ...
    function join(&$rightTable, $foreignKey)
    {
        ...
        reset($this->data);
        while (list ($id) = each($this->data)) {
            if (strcasecmp($foreignKey, "id") == 0)
                $fid = $id;
            else
                $fid = $this->data[$id][$foreignKey];
    ...
    function each()
    {
        ...
        if (isset($this->_limitRows) && -- $this->_limitRows < 0)
            return false;
        if (! list ($id, $data) = each($this->data))
            return false;
        return array(
            'id' => $id
        ) + $data;
     ...
     * This depends on how's the table
     ...
     * Use <i>offset</i> to get another ID near to
     ...
     * Use <i>offset</i> to get another ID near to the
        ...
        foreach ($this->_cmpFields as $f) {
            ...
            switch ($f['type'] & ~ SORT_NULL) {
                ...
                default:
                    ...
                    $result = ($a[$f['field']] == $b[$f['field']]) ? 0 : ($a[$f['field']] > $b[$f['field']] ? $f['order'] : - $f['order']);
                    break;
            }
    ...
    function data($id)
    {
        return isset($this->data[$id]) ? array(
            'id' => $id
        ) + $this->data[$id] : false;
    }
    ...
    function export()
    {
        ...
        reset($this->data);
        while (list ($id, $row) = each($this->data)) {
            if (strpos($id, $this->delimiter) === false && strpos($id, '"') === false) {
                $csv .= $id;
            } else {
                $csv .= '"' . str_replace('"', '""', $id) . '"';
            }
    ...
    {
        if (! isset($whence))
            $id = array_search($id, array_keys($this->data));
