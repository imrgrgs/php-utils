...
class Apeform
{
     ...
     * Switches to "multipart/form-data" when using
    ...
    /**
     * Defaults to "form".
     * If set to something else, this will be used as prefix
    ...
    /**
     * Defaults to the value of {@link id} which defaults to "form".
     * This means,
     ...
     * {@link text()} and {@link password()} elements.
     * Default is 255
    ...
    /**
     * Default width is 40 characters.
     * This can also be changed when calling
     * {@link Apeform()} or using an extended class. It's also possible to set
     ...
     * element because PHP will remove any temporary file immediately.
     * Set this
    ...
    /**
     * The class uses it's own tiny HTML templating system.
     * This associative
    ...
    var $templates = array(
        ...
        'label' => "{label}:",
        'error' => '<strong class="error">{error}:</strong>',
        ...
        'accesskey' => '<span style="text-decoration:underline">{accesskey}</span>'
    ...
    {
        $this->_rows[count($this->_rows)] = array(
            'type' => "header",
            'header' => $header,
            'name' => false
        );
     ...
     * @return mixed
     */
    function &text($label = "", $help = "", $defaultValue = "", $maxLength = 0, $size = 0)
    {
        $count = max(count($defaultValue), count($maxLength), count($size));
        if (is_string($help))
            $help = explode("\t", $help, 1 + $count);
        if ($count > 1) {
            $defaultValue = (array) $defaultValue;
            for ($i = 1; $i < $count; ++ $i) {
                if (! isset($help[$i]))
                    $help[$i] = "\n";
                if (! isset($defaultValue[$i]))
                    $defaultValue[$i] = "";
            }
        }
        $id = count($this->_rows);
     ...
     * @return string
     */
    function &password($label = "", $help = "", $defaultValue = "", $maxLength = 0, $size = 0)
    {
        $id = count($this->_rows);
        $name = ($this->id == "form" ? "" : $this->id) . "element" . $id;
        $this->_rows[$id] = array(
     ...
     * @return string
     */
    function &textarea($label = "", $help = "", $defaultValue = "", $rows = 0, $cols = 0, $wrap = "virtual")
    {
        $id = count($this->_rows);
        $name = ($this->id == "form" ? "" : $this->id) . "element" . $id;
        $this->_rows[$id] = array(
    ...
    function &checkbox($label, $help = "", $options = "", $defaultValue = "")
    {
        ...
        if (count($this->_rows[$id]['options']) > 1) {
            ...
            $this->_rows[$id]['value'] = $this->_isSubmitted ? array() : $this->_explodeOptions($defaultValue);
        } else {
    ...
    function &radio($label, $help, $options, $defaultValue = "")
    {
        ...
        if (! isset($this->_rows[$id]['options'][$this->magicQuotes ? addslashes($defaultValue) : $defaultValue])) {
            $this->_rows[$id]['value'] = array_search($defaultValue, $this->_rows[$id]['options']);
            if ($this->magicQuotes) {
    ...
    function &file($label = "", $help = "", $size = 0)
    {
        ...
        if (isset($_FILES[$name]) && $_FILES[$name]['size']) {
            ...
            if (get_magic_quotes_gpc())
                $postedName = stripslashes($postedName);
            if ($this->magicQuotes)
                $postedName = addslashes($postedName);
        } // Read meta data from the hidden element if present.
        elseif (isset($_POST[$name . "h"])) {
            $this->_rows[$id]['value'] = unserialize(stripslashes($_POST[$name . "h"]));
        } elseif (isset($GLOBALS['HTTP_POST_VARS'][$name . "h"])) {
            $this->_rows[$id]['value'] = unserialize(stripslashes($GLOBALS['HTTP_POST_VARS'][$name . "h"]));
        }
        ...
        if ($this->_rows[$id]['value']) {
            $this->_rows[$id]['value']['unixname'] = $this->_getUnixName($this->_rows[$id]['value']['name']);
            // Fix for a bug in IE which returns "pjpeg" for progressive JPEGs.
            $this->_rows[$id]['value']['type'] = preg_replace('{^image\W\wjpe?g$}is', 'image/jpeg', $this->_rows[$id]['value']['type']);
            ...
            if (is_uploaded_file($this->_rows[$id]['value']['tmp_name'])) {
                ...
                if (! $this->_doGarbageCollection($tempnam)) {
                    user_error("Apeform::file() failed, tmpDir is not set properly", E_USER_WARNING);
     ...
     *
     * @param
     *            name string The filename to be processed.
     * @param
     *            maxLength int Maximum filename length, defaults to 64 (Joliet).
     * @return string
     * @access private
     */
    function _getUnixName($name, $maxLength = 64)
    {
        ...
        $name = strtr($name, "\x80\x83\x8A\x8C\x8E\x96\x97\x9A\x9C\x9E\x9F¢£¥©ª­®²³µ¹º" . "ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖ×ØÙÚÛÜÝÞàáâãäåæçèéêëìíîïðñòóôõöøùúûüýþÿ", "EfSOZ--sozYcLYca-r23u1o" . "AAAAAAACEEEEIIIIDNOOOOOxOUUUUyTaaaaaaaceeeeiiiidnoooooouuuuyty");
    ...
    /**
     * Remove some old temporary files which exceeded the timeout.
     * Default for
    ...
    function submit($value = "", $help = "")
    {
        ...
        if (isset($postedValue)) {
            ...
            if (empty($value))
                $postedValue = true;
            // Deny unknown values.
            elseif (! in_array($postedValue, $this->_rows[$id]['value']))
                return false;
            elseif ($this->magicQuotes)
    ...
    {
        if (! $this->autoAccesskeys || stristr($label, "<u>"))
            return $label;
    ...
    function &_fetchPostedValue()
    {
        ...
        if (isset($postedValue)) {
            if (get_magic_quotes_gpc()) {
                $postedValue = is_array($postedValue) ? array_map('stripslashes', $postedValue) : stripslashes($postedValue);
            }
                    ...
                    foreach ((array) $postedValue as $i => $v) {
                        ...
                        if (strlen($v) > $l) {
                            ...
                            if (is_array($postedValue))
                                $postedValue[$i] = substr($v, 0, $l);
                            else
                                $postedValue = substr($v, 0, $l);
                        }
                    }
                    break;
                case "checkbox":
        ...
        if ($this->magicQuotes) {
            $element['value'] = is_array($element['value']) ? array_map('addslashes', $element['value']) : addslashes($element['value']);
        }
    ...
    {
        $form = '<form action="';
        $form .= isset($_SERVER['PHP_SELF']) ? $_SERVER['PHP_SELF'] : $GLOBALS['HTTP_SERVER_VARS']['PHP_SELF'];
        ...
        while (list ($id, $row) = each($this->_rows)) {
            ...
            if (! empty($row['help'][0])) {
                $genericAttr .= ' title="' . str_replace('"', '&quot;', strip_tags($row['help'][0])) . '"';
            }
            $input = '';
            switch ($row['type']) {
                case "header":
                ...
                case "hidden":
                    ...
                    $form .= '<input type="hidden"' . $nameAttr . ' value="' . htmlspecialchars($this->magicQuotes ? stripslashes($row['value']) : $row['value']) . '" />';
                    continue 2;
                case "static":
                    $input = nl2br(htmlspecialchars($row['value'])) . '<input type="hidden"' . $nameAttr . ' value="' . htmlspecialchars($this->magicQuotes ? stripslashes($row['value']) : $row['value']) . '" />';
                    break;
                case "textarea":
                    ...
                    $input .= htmlspecialchars($this->magicQuotes ? stripslashes($row['value']) : $row['value']);
                    $input .= '</textarea>';
                    break;
                case "select":
                    ...
                    while (list ($key, $value) = each($row['options'])) {
                        $input .= '<option value="';
                        $input .= htmlspecialchars($this->magicQuotes ? stripslashes($key) : $key);
                        $input .= '"';
                        if (strcmp($key, $row['value']) == 0)
                ...
                case "checkbox":
                    ...
                    while (list ($key, $value) = each($row['options'])) {
                        ...
                        $input .= '" value="' . htmlspecialchars($this->magicQuotes ? stripslashes($key) : $key);
                        $input .= '"';
                ...
                case "submit":
                    while (list ($i, $value) = each($row['value'])) {
                        ...
                        if (! empty($value)) {
                            $input .= ' value="' . strtr($this->magicQuotes ? stripslashes($value) : $value, array(
                                '"' => '&quot;',
                                '<' => '&lt;',
                                '>' => '&gt;'
                            )) . '"';
                ...
                case "image":
                    while (list ($i, $value) = each($row['value'])) {
                        $input .= '<input type="image"' . $nameAttr . ' src="' . htmlspecialchars($this->magicQuotes ? stripslashes($value) : $value) . '"' . $genericAttr . " />\n";
                    }
                    break;
                case 'text':
                    $c = count($row['value']);
                    ...
                    foreach ((array) $row['value'] as $i => $value) {
                        ...
                        if ($c > 1)
                            $input .= '[]';
                        $input .= '" value="';
                        $input .= htmlspecialchars($this->magicQuotes ? stripslashes($value) : $value);
                        $input .= '" maxlength="' . ($maxes[$i] ? $maxes[$i] : $this->maxLength) . '"';
                ...
                default:
                    ...
                    if (isset($row['maxLength'])) {
                        $input .= ' maxlength="' . ($row['maxLength'] ? $row['maxLength'] : $this->maxLength) . '"';
                    }
                    ...
                    if ($row['type'] == "file" && ! empty($row['value'])) {
                        ...
                        $input .= ' value="' . htmlspecialchars(serialize($row['value'])) . '" />';
                    }
            }
            if (! empty($row['help'][1]))
                $input .= $row['help'][1];
            ...
            if (isset($row['error']) && strpos($element, "{error}") === false) {
                ...
                if (! stristr($row['error'], "<u>") && preg_match('/<u>(\w)/i', @$row['label'], $match)) {
                    $row['error'] = preg_replace('/' . $match[1] . '/i', '<u>\0</u>', $row['error'], 1);
                }
            ...
            if (! (empty($row['label']) && empty($row['error'])) && strpos($input, "<label") === false && preg_match('/id="([^"]+)/', $input, $mId)) {
                $labelTag = '<label';
                ...
                $labelTag .= ' for="' . $mId[1] . '">';
                $element = str_replace("{label}", $labelTag . "{label}</label>", $element);
            }
            if (isset($row['error']) && strpos($element, "{error}") === false) {
            ...
            if (! empty($row['label']) && $this->getTemplate('label', $row['name'])) {
                $element = str_replace("{label}", $this->getTemplate('label', $row['name']), $element);
            }
            if (! empty($row['error']) && $this->getTemplate('error', $row['name'])) {
                $element = str_replace("{error}", $this->getTemplate('error', $row['name']), $element);
            }
            if (! empty($row['help'][0]) && $this->getTemplate('help', $row['name'])) {
                $element = str_replace("{help}", $this->getTemplate('help', $row['name']), $element);
            }
            $elements .= str_replace(array(
                "{label}",
                "{error}",
     ...
     *
     * @param
     *            template string One of the templates names.
     * @param
     *            name string Element name currently rendered.
     * @return string
     */
    ...
    function _getFocusHandler()
    {
        ...
        for ($i = 0; $i < $count; $i ++) {
            ...
            if ($this->_rows[$i]['type'] != 'text' && $this->_rows[$i]['type'] != 'password' && $this->_rows[$i]['type'] != 'textarea')
                continue;
            if (! isset($id))
                $id = $i;
            // Search for the first text element containing an error.
            if (! empty($this->_rows[$i]['error'])) {
                $id = $i;
                break;
            }
        }
