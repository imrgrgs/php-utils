...
class MyCSVAdmin
{
     ...
     * functions of the administrative interface.
     * Default is PRIV_ALL,
     * everything enabled.
     *
    ...
    /**
     *
     * @var MyCSV MyCSV object currently edited by MyCSVAdmin.
     */
    var $table = null;
     ...
     * Loads the table specified by $_GET['table'] or
     ...
     * This unifies the different behaviours of $_GET,
     * $HTTP_GET_VARS, $GLOBALS (register_globals) and so on.
     *
     * @param string $key
     *            Name of the request variable to be returned.
     * @return string
     */
    function GET($key)
    {
    ...
    function htmlQuote($text)
    {
        ...
        $text = preg_replace('/[\x00-\x08\x0B-\x0C\x0E-\x1F\x7F\x81\x83\x86-\x8F\x90\x98-\x9F]/', '.', $text);
     ...
     * Use {@link dir} to specify the working
     * directory.
     *
     * @return void
     */
    function tables()
    {
        ...
        foreach ((array) $this->dir as $dir) {
            ...
            while (($filename = readdir($fp)) !== false) {
                ...
                if (! is_file($filename))
                    continue;
                if (! preg_match('/\.([a-z]sv|txt)$/i', $filename))
                    continue;
                $tables[] = $filename;
            }
            closedir($fp);
